...
def view_orders() -> None:
    ...
    while is_looping:
        ...
        print(fmt_string('Enter 0 to return to the previous menu!', fg='Green'))
        res, value = validated_input(
            'Select An Order To View [ID] Or Filter [-1]: ', int, fg='Green', min_value=-1, min_length=1, max_value=len(orders_data))
        ...
        if value == -1:
            filter_options = [{
                ...
                'choices': ['Clear Filter', Separator(), 'Name', 'Address', 'Area', 'Number', 'Status'],
...
def update_status(index=None) -> None:
    ...
    while is_looping:
        ...
        if orders_data[value - 1]['courier'] == -1:
            ...
            questions = [
                {
                    ...
                    'choices': choices + [val.title() for val in get_external_data('couriers')],
                    'filter': lambda val: val.lower()
                }
            ]
            ...
            orders_data[value - 1]['courier'] = get_external_data(
                'couriers').index(answers['courier'])
...
def select_order_items(current_order: list = []) -> list[int]:
    ...
    while is_looping:
        ...
        print_table('Current Order', summerise_list(current_items))
        ...
        if update_type_question['type'].lower() == 'skip':
            is_looping = False
            continue
        ...
        if update_type_answer['type'] == 'add':
            ...
            while loop_cat:
                ...
                while loop_items:
                    clear()
                    print_table('Current Order', summerise_list([get_external_data('products')[
                                index].title() for index in current_order]))

                    choices = ['Back', Separator()]
                    for index in catagory_mapping[catagory_choice_answer['catagory']]:
                        item_name = get_external_data(
                    ...
                    current_order.append(get_external_data(
                        'products').index(items_choice_answers['items'].lower()))
            ...
            for item in list(set(list_to_remove)):
                ...
                current_order.remove(get_external_data(
                    'products').index(item.lower()))
...
# TODO - Need to add to reference updates AND add to config and load in
catagory_mapping = {
