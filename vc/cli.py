...
def validated_input(prompt: str, *types, input=input, **options) -> tuple[int, Any]:
    ...
    if 'cancel_condition' in options:
        cancel_text = fmt_string(f'Enter {options["cancel_condition"]} To Cancel\n', fg='Green')

    if 'fg' in options:
        value = str(input(fmt_string(cancel_text + prompt, fg=options['fg']))).lower()

    elif 'bg' in options:
        value = str(input(fmt_string(cancel_text + prompt,
                                 fg=options['bg']))).lower()

    elif 'fg' in options and 'bg' in options:
        value = str(input(fmt_string(
            cancel_text + prompt, fg=options['fg'], bg=options['bg']))).lower()

    else:
        value = str(input(fmt_string(cancel_text + prompt))).lower()
        
    if 'cancel_condition' in options:
        if value == options['cancel_condition']:
...
def get_validated_input(questions: list[dict[str, Any]], input_dependancy=input) -> dict[str, str]:
    result = {}
    for question in questions:
        ...
        while res != 1:
            ...
            res, value = validated_input(
                question['prompt'], question['types'], question['options'], input=input_dependancy)
            if res == -1:
                return {}
            continue
